...
func main() {
	...
	http.HandleFunc("/slack/team/auth", TeamAuthorize)
	http.HandleFunc("/slack/team/callback", TeamCallback)

	http.HandleFunc("/slack/user/auth", UserAuthorize)
	http.HandleFunc("/slack/user/callback", UserCallback)

	http.ListenAndServe(":8080", nil)
}

func TeamAuthorize(w http.ResponseWriter, r *http.Request)  {
...
func UserAuthorize(w http.ResponseWriter, r *http.Request)  {
	state := uuid.New().String()
	scopes := []string{"users.identity"}
	c := auth.NewUserAuth(ClientID, CallbackURL, state, scopes)
	...
	// https://slack.com/oauth/v2/authorize?client_id=123456789.14712345678&redirect_uri=http%3A%2F%2Flocalhost%3A8080%2Fslack%2Fcallback&user_scope=users.identity&state=1be65a10-1f6b-4013-8c85-03f728d5ba7d
	c.Redirect(w, r)
}
...
func TeamCallback(w http.ResponseWriter, r *http.Request) {
